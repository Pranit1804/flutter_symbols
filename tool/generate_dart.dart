...
void main() {
  ...
  final outputFile = File(
    '${repoRoot.path}/lib/flutter_cupertino_symbols.dart',
  );
  ...
  final codepointMap =
      (jsonDecode(codepointFile.readAsStringSync()) as Map<String, dynamic>)
  ...
  for (final entry in codepointMap.entries) {
    ...
    if (entries.containsKey(dartId)) {
      ...
      var i = 2;
      while (entries.containsKey('${dartId}_$i')) {
        i++;
      }
      dartId = '${dartId}_$i';
    }
  ...
  buf.writeln('//');
  buf.writeln(
    '// IMPORTANT: SF Symbols are licensed by Apple for use on Apple platforms',
  );
  buf.writeln(
  ...
  buf.writeln('///');
  buf.writeln(
    '/// **License notice**: These icons are licensed by Apple for use on',
  );
  buf.writeln(
